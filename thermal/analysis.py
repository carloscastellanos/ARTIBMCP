...
def sendContour(
    id, perimeter, area, circularity, solidity, addr="/thermal/leaf/contour"
):
...
def main():
    ...
    img[0 : int(y), img.shape[1] - int(x) : img.shape[1]] = np.zeros(
        (int(y), int(x)), dtype=np.uint8
    )  # top right
    img[
        img.shape[0] - int(y) : img.shape[0], img.shape[1] - int(x) : img.shape[1]
    ] = np.zeros(
        (int(y), int(x)), dtype=np.uint8
    )  # bot right
    img[img.shape[0] - int(y) : img.shape[0], 0 : int(x)] = np.zeros(
        (int(y), int(x)), dtype=np.uint8
    )  # bot left
    ...
    for i in range(len(contours)):
        ...
        if hierarchy[0][i][3] == -1:
            ...
            cv2.putText(
                out, str(i), (x, y - 1), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 0), 1
            )
            print("contour " + str(i) + ":")
